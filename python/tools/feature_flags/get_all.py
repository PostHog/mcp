...
async def get_all_feature_flags_handler(
    context: Context, _params: FeatureFlagGetAllSchema
) -> ToolResult:
    ...
    return ToolResult(
        content=[TextContent(text=json.dumps([flag.model_dump() for flag in flags_result.data]))]
    )
