...
describe('API Client Integration Tests', () => {
    ...
    beforeAll(async () => {
        ...
        try {
            ...
            if (!createOrgResult.ok) {
                throw new Error(`Failed to create test organization: ${createOrgResult.statusText}`);
            }
        ...
        try {
            const createProjectResult = await fetch(`${API_BASE_URL}/api/organizations/${testOrgId}/projects/`, {
                method: 'POST',
                headers: {
                    'Authorization': `Bearer ${API_TOKEN}`,
                    'Content-Type': 'application/json',
    ...
    afterAll(async () => {
        ...
        if (testOrgId) {
            ...
            try {
                const deleteResult = await fetch(`${API_BASE_URL}/api/organizations/${testOrgId}/`, {
                    method: 'DELETE',
                    headers: {
                        'Authorization': `Bearer ${API_TOKEN}`,
                    },
    ...
    describe('Projects API', () => {
        ...
        it('should get property definitions', async () => {
            const result = await client.projects().propertyDefinitions({ projectId: testProjectId });
    ...
    describe('Insights API', () => {
        ...
        it('should create, get, update, and delete an insight', async () => {
            ...
            if (createResult.success) {
                ...
                // Get
                const getResult = await client.insights({ projectId: testProjectId }).get({ insightId });
